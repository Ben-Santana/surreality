...
const MAX_FILES = 512;
const CORE_MEDIA_PACKAGE_ID = "room.mapping.media";
...
export function uninstallCustomMapping(app: App, id: string, version: string): boolean {
  if (id === CORE_MEDIA_PACKAGE_ID) return false;
  const target = packageDirectory(app, id, version);
