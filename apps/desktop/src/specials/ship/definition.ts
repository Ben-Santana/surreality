...
import { LiveShipBullets, LiveShipDocks, LiveShipExhaust, LiveShipMinigame } from "./ShipBullets";
import { idleShipGame, useShipPlayStore } from "./playStore";
import { createElement, Fragment } from "react";

function ShipRuntimeHost() { useShipRuntime(); return null; }
function ShipOverlay() { return createElement(Fragment, null, createElement(LiveShipDocks), createElement(LiveShipMinigame), createElement(LiveShipExhaust), createElement(LiveShipBullets)); }
...
export const shipSpecial: SpecialDefinition<ShipConfig> = {
  ...
  runtime: {
    ...
    getSnapshot: () => {
      const { bullets, charges, exhaust, docks, game } = useShipPlayStore.getState();
      return { bullets, charges, exhaust, docks, game };
    },
    applySnapshot: (value) => {
      if (!value || typeof value !== "object") return;
      const play = value as ReturnType<typeof useShipPlayStore.getState>;
      useShipPlayStore.getState().setPlay({ bullets: play.bullets ?? [], charges: play.charges ?? {}, exhaust: play.exhaust ?? [], docks: play.docks ?? {}, game: play.game ?? idleShipGame });
    },
  },
  View: ShipView,
