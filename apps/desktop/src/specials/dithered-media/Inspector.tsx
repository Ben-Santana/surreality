...
export function DitheredMediaInspector({ config, onChange }: SpecialInspectorProps<DitheredMediaConfig>) {
  ...
  const pick = async () => {
    ...
    try {
      ...
      const gif = result.asset.mimeType === "image/gif";
      const image = result.asset.mimeType.startsWith("image/");
      onChange({ ...current, source: result.asset.source, fileName: result.asset.fileName, mediaType: gif ? "gif" : image ? "image" : "video" });
      setUrl("");
...
function Select({ label, value, items, onChange }: { label: string; value: string; items: { id: string; label: string }[]; onChange: (value: string) => void }) { return <div className="space-y-1.5"><span className="chrome-label">{label}</span><InspectorSelect ariaLabel={label} value={value} options={items} onChange={onChange} /></div>; }
