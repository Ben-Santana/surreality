...
if (!process.argv[2] || !fs.existsSync(directory)) {
  console.error("Usage: npm run pack:mapping -- ./custom-mappings/my-mapping [output.mapping]");
  process.exit(1);
}
...
function collect(current) {
  for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && !entry.name.endsWith(".mapping")) {
...
const output = process.argv[3]
  ...
  : path.join(directory, `${manifest.id}-${manifest.version}.mapping`);
